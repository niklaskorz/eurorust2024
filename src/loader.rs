...
use deno_core::{
    resolve_import, resolve_path, ModuleLoadResponse, ModuleLoader, ModuleSource, ModuleSourceCode,
...
impl ModuleLoader for TypescriptModuleLoader {
    fn resolve(
        ...
        if specifier.starts_with(INTERNAL_MODULE_PREFIX) {
            ...
            return Ok(resolve_path(&path_str, &env::current_dir()?)?);
        }
    ...
    fn load(
        ...
        // from the network.
        ModuleLoadResponse::Sync(self.load_sync(
            module_specifier,
            maybe_referrer,
            is_dyn_import,
